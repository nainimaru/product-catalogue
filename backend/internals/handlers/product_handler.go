...
type ProductRequest struct {
    ...
    Category    string           `json:"category"`    // plain string from client
    Images      []string         `json:"images"`
    Price       int              `json:"price"`
    Tags        []models.KeyValue  `json:"tags"`
    Specs       []models.KeyValue  `json:"specs"`
    Description string           `json:"description"`
    Brand       string           `json:"brand"`
    Stock       int              `json:"stock"`
}

func resolveCategory(ctx context.Context, name string) (primitive.ObjectID, error) {
    slug := strings.ToLower(strings.TrimSpace(name))

    var existing models.Category
    err := categoryCollection.FindOne(ctx, bson.M{"slug": slug}).Decode(&existing)

    if err == nil {
        // found category in categories collection, return existing ID
        return existing.ID, nil
    }

    if err != mongo.ErrNoDocuments {
        // As err is neither nil nor No Document error that means some other thing went wrong
        return primitive.NilObjectID, err
    }

    // category not found i.e. err = mongo.ErrNoDocuments, create it
...
func AddProduct(w http.ResponseWriter, r *http.Request) {
    ...
    categoryID, err := resolveCategory(r.Context(), req.Category)
    if err != nil {
